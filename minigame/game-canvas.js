/**
 * 八卦立方体 - Canvas 2D 渲染模式
 * 原始的 Canvas 实现，轻量级，兼容性好
 */

// 从原始 game.js 复制的完整实现
// 这个文件保留原始的 Canvas 2D 渲染逻辑
...
function initCanvasGame() {
  ...
  // =============== 火柴人与场景 ===============
  let walkTime = 0;
  const CUBE_SIZE = GameConfig.common.bagua.cubeSize;
  let sceneOffset = 0;

  let stickManSpeed = GameConfig.canvas.stickMan.initialSpeed;
  let targetSpeed = GameConfig.canvas.stickMan.initialSpeed;
  const SPEED_LERP = 0.05;
  const BASE_SCENE_SPEED = GameConfig.canvas.scene.moveSpeed;
  ...
  const POSE_LERP = GameConfig.canvas.stickMan.smoothness;
  ...
  function draw() {
    ...
    // 显示模式指示
    ctx.fillText('Canvas 2D 模式', 15, H - 15);
  }
  ...
  // =============== 启动 ===============
  console.log('八卦立方体 Canvas 模式初始化...');
  requestAnimationFrame(gameLoop);
  console.log('游戏循环启动');
  ...
  return {
    ...
    getPalace: () => currentPalace,
  };
}

module.exports = { initCanvasGame };
