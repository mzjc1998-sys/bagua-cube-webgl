...
class Player {
  constructor(x, y) {
    ...
    this.radius = 12;
    this.color = '#4CAF50';
    this.facingAngle = 0; // 面朝方向（实际渲染使用）
    this.targetFacingAngle = 0; // 目标朝向（移动方向）
    this.facingSmoothing = 0.15; // 朝向平滑系数（越小越平滑，0.08-0.2为推荐范围）
  ...
  setMoveTarget(targetX, targetY) {
    ...
    // 计算目标朝向（不直接设置 facingAngle，而是设置目标值让其平滑过渡）
  ...
  /**
   * 平滑更新朝向（角度插值，处理 -π 到 π 的环绕）
   */
  updateFacingAngle(deltaTime) {
    // 计算角度差（处理环绕）
    ...
    // 平滑插值（基于时间的平滑）
    // 使用指数衰减，让转向有自然的缓入感
    const smoothing = 1 - Math.pow(1 - this.facingSmoothing, deltaTime / 16);
    this.facingAngle += diff * smoothing;
  ...
  update(deltaTime, dungeon, collisionManager = null) {
    ...
    // === 平滑更新朝向（始终执行，让转向更自然）===
    this.updateFacingAngle(deltaTime);
    ...
    if (this.isMoving && !this.isAttacking) {
      ...
      if (dist > 0.1) {
        ...
        // 更新目标朝向（使用平滑过渡，不直接设置）
        if (moveX !== 0 || moveY !== 0) {
          this.targetFacingAngle = Math.atan2(moveY, moveX);
        }
      } else {
        this.isMoving = false;
      }
